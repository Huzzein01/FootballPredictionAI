...
- Team-only parlays include high draw-risk fixtures as eligible result picks, but draw legs are sampled rather than forced into every ticket
- Multi-page navigation separates predictions, parlays, parlay backtesting, played matches, single-match prediction, and the fixture ledger
...
Fixture Ledger settlement is also automated. `/api/backtests` and `/api/fixtures/espn-results-refresh` check ESPN public scoreboards for recently completed club fixtures, settle matching pending fixture-board predictions, refresh table context, and queue continuous retraining when new final scores are imported.
