...
function statLine(state, leaguePpg, now) {
  ...
  const restDays = state?.lastPlayedAt && now ? Math.max(0, Math.min(21, Math.round((now - state.lastPlayedAt) / 86_400_000))) : 7;
  return {
...
// Same leakage-safe chronological replay used by baseballModel and
// basketballModel — every snapshot is built from strictly-earlier results.
function reconstructTrainingRows(games) {
  ...
  for (const game of completed) {
    ...
    const snapshot = {
      ...
      kickoffUtc: game.kickoffUtc || `${game.date}T18:00:00Z`,
      capturedAt: isoBefore(game.kickoffUtc || `${game.date}T18:00:00Z`),
      ...
      provenance: {
        type: "historical_reconstruction", cutoff: "strictly-before-kickoff", sourceGameId: game.id,
        limitations: ["No starter, injury, weather, or travel fields are included — ratings are results-only."],
      },
    };
