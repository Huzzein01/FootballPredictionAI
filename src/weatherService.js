...
// Live per-fixture forecasts generated by scripts/fetchVenueWeather.js
const forecastsRaw = readJsonWithFallback(
  repoDataPath("international", "venue_weather_forecasts.json"), null, null
);

const VENUES    = venuesRaw?.venues    ?? {};
const PROFILES  = profilesRaw?.profiles ?? {};
// Keyed by matchNumber (string or number); values have .heatTier, .effectiveTempC, .source
const FORECASTS = forecastsRaw?.byMatch ?? {};
...
if (!Object.keys(FORECASTS).length) {
  console.warn("[weatherService] ℹ  venue_weather_forecasts.json not found — run scripts/fetchVenueWeather.js for live forecasts.");
}
...
/**
 * Effective heat index for a venue.
 * When a live/forecast record exists (from venue_weather_forecasts.json) it takes
 * priority; otherwise falls back to the static profile with time-of-day reduction.
 *
 ...
 * @param {number|string|null} matchNumber — used to look up the live forecast cache
 * @returns {number}
 */
function venueEffectiveHeat(city, kickoffLocalIso, matchNumber) {
  // Prefer live forecast heat tier if available
  if (matchNumber != null) {
    const fc = FORECASTS[matchNumber] ?? FORECASTS[String(matchNumber)];
    if (fc?.heatTier != null) return fc.heatTier;
  }

  // Fall back to static venue profile with time-of-day reduction
  const v = VENUES[city];
  if (!v) return 3;
 ...
 * @param {number|string|null} matchNumber
 * @returns {number}
 */
function heatAdjForTeam(team, city, kickoffLocalIso, matchNumber) {
  const venueHeat = venueEffectiveHeat(city, kickoffLocalIso, matchNumber);
...
function weatherContextForFixture(fixture) {
  ...
  const matchNumber = fixture.matchNumber  ?? null;
  const v           = VENUES[city] ?? {};

  // Live forecast record (if available)
  const fc = matchNumber != null
    ? (FORECASTS[matchNumber] ?? FORECASTS[String(matchNumber)] ?? null)
    : null;

  const venueTier = venueEffectiveHeat(city, kickoff, matchNumber);

  const homeClimate = teamClimateScore(fixture.homeTeam);
  const awayClimate = teamClimateScore(fixture.awayTeam);

  const homeHeat = heatAdjForTeam(fixture.homeTeam, city, kickoff, matchNumber);
  const awayHeat = heatAdjForTeam(fixture.awayTeam, city, kickoff, matchNumber);
  ...
  return {
    ...
    hasAC:            v.hasAC            ?? false,
    // Live forecast fields (null when no forecast available)
    forecastTempC:    fc?.effectiveTempC  ?? null,
    forecastHumidity: fc?.humidity        ?? null,
    forecastSource:   fc?.source          ?? "static-profile",
    home,
    away,
