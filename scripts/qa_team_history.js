...
const files = fs.existsSync(historyDir()) ? fs.readdirSync(historyDir()).filter((file) => file.endsWith(".json") && file !== "_index.json" && readHistory(path.basename(file, ".json"))?.contract === "football-team-history-v1") : [];
