...
for (const file of fs.readdirSync(historyDir()).filter((name) => name.endsWith(".json") && name !== "_index.json")) { const record = readHistory(path.basename(file, ".json")); const before = record.matches.length; record.matches = record.matches.filter((match) => !/^football-data:[a-f0-9]{64}:.+:(home|away)$/.test(match.id || "")); if (record.matches.length !== before) { removed += before - record.matches.length; teams += 1; rebuildCoverage(record); record.updatedAt = new Date().toISOString(); writeHistory(record); } }
